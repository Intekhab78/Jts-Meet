...
export async function createTeam(userId: string, payload: {
    ...
    const existingTeam = await Team.findOne({ organizationId: organization._id, name: payload.name.trim(), deletedAt: null }).exec()
    if (existingTeam) {
    ...
    await createGeneralChannel(userId, savedTeam.organizationId.toHexString(), savedTeam._id.toHexString())
    return savedTeam
}
